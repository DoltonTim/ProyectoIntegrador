...
public class Procesocontroller {
    ...
    public ResponseEntity<List<Proceso>> readAll() {
        try {
            List<Proceso> lista = service.readAll();
            if (lista.isEmpty()) {
    ...
    @PostMapping
    public ResponseEntity<Proceso> create(@Valid @RequestBody Proceso obj) {
        try {
        	Proceso objeto = service.create(obj);
    ...
    public ResponseEntity<Proceso> getProcesoById(@PathVariable Long id) {
        try {
            Optional<Proceso> objeto = service.read(id);
            if (objeto.isPresent()) {
    ...
    @PutMapping("/{id}")
  	public ResponseEntity<Proceso> updatecliente(@PathVariable Long id, @Valid @RequestBody Proceso obj) {
  	    Optional<Proceso> objeto = service.read(id);
  	    if (objeto.isPresent()) {
  	        // Aquí se asume que el servicio actualiza el objeto y lo retorna
  	    	Proceso actualizado = service.update(obj);
