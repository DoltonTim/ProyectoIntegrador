...
public class Rubrocontroller {
    ...
    public ResponseEntity<List<Rubro>> readAll() {
        try {
            List<Rubro> lista = service.readAll();
            if (lista.isEmpty()) {
    ...
    @PostMapping
    public ResponseEntity<Rubro> create(@Valid @RequestBody Rubro obj) {
        try {
        	Rubro objeto = service.create(obj);
    ...
    public ResponseEntity<Rubro> getRubroById(@PathVariable Long id) {
        try {
            Optional<Rubro> objeto = service.read(id);
            if (objeto.isPresent()) {
    ...
    @PutMapping("/{id}")
  	public ResponseEntity<Rubro> updateRubro(@PathVariable Long id, @Valid @RequestBody Rubro obj) {
  	    Optional<Rubro> objeto = service.read(id);
  	    if (objeto.isPresent()) {
  	        // Aquí se asume que el servicio actualiza el objeto y lo retorna
  	    	Rubro actualizado = service.update(obj);
