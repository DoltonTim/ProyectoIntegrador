...
public class Usuariocontroller {
    ...
    public ResponseEntity< Usuario> getUsuarioById(@PathVariable Long id) {
        try {
            Optional< Usuario> objeto = service.read(id);
            if (objeto.isPresent()) {
    ...
    @PutMapping("/{id}")
  	public ResponseEntity< Usuario> updatecliente(@PathVariable Long id, @Valid @RequestBody  Usuario obj) {
  	    Optional< Usuario> objeto = service.read(id);
  	    if (objeto.isPresent()) {
  	        // Aquí se asume que el servicio actualiza el objeto y lo retorna
  	    	 Usuario actualizado = service.update(obj);
