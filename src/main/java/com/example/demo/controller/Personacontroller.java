...
public class Personacontroller {
    ...
    public ResponseEntity<Persona> create(@Valid @RequestBody Persona obj) {
        try {
        	Persona objeto = service.create(obj);
    ...
    public ResponseEntity<Persona> getPersonaById(@PathVariable Long id) {
        try {
            Optional<Persona> objeto = service.read(id);
            if (objeto.isPresent()) {
    ...
    @PutMapping("/{id}")
  	public ResponseEntity<Persona> updatecliente(@PathVariable Long id, @Valid @RequestBody Persona obj) {
  	    Optional<Persona> objeto = service.read(id);
  	    if (objeto.isPresent()) {
  	        // Aquí se asume que el servicio actualiza el objeto y lo retorna
  	    	Persona actualizado = service.update(obj);
